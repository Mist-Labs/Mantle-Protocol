...
contract PrivateSettlement is ReentrancyGuard {
    ...
    event WithdrawalClaimed(bytes32 indexed intentId, bytes32 indexed nullifier, address recipient);
    ...
    constructor(address _relayer, address _feeCollector, address _poseidonHasher) {
        relayer = _relayer;
    ...
    function fillIntent(
        ...
        if (!_verifySourceCommitment(commitment, sourceChain, sourceRoot, merkleProof, leafIndex)) {
            revert InvalidProof();
        }
    ...
    function claimWithdrawal(
        ...
        bytes32 authHash = keccak256(abi.encodePacked(intentId, nullifier, recipient));
        ...
        if (computedCommitment != intentCommitments[intentId]) revert InvalidCommitment();
